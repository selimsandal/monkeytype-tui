...
Press F10 or click **settings** to open the menu. Use Up/Down to select, Left/Right or Enter to change, and Esc to close; clicking a setting cycles its value. Language choices are English, Spanish, French, and German, with word lists and quotes from Monkeytype. Punctuation and numbers affect generated time/words tests, not quotes or custom text. The menu also exposes difficulty, stop/delete on error, confidence, strict space, freedom, and quick end. Changing a setting or opening the menu during an active test starts a fresh test. Choices are saved to your user config directory (`~/.config/monkeytype-tui/settings.json` on Linux/macOS, `%APPDATA%\monkeytype-tui\settings.json` on Windows).
